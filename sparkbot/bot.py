...
class SparkBot:
    ...
    def command_dispatcher(self, user_message, callback):
        """ Dispatches a command for ``user_message``

        :param user_message: The text of the message sent by the user when calling the
                             bot.

        :param callback: Called with each response generated by the user's desired
                         command
        ...
        if isinstance(command_response, str):
            callback(command_response)
        elif isinstance(command_response, GeneratorType):
            for response in command_response:
                callback(response)
        else:
            raise CommandRunError("Command did not return or yield a str.")
